        ...
        <div className={"w-screen h-screen fixed top-0 left-0 z-20 flex" + visibleClass} style={{ backgroundColor: 'rgba(0,0,0,.6)' }}>
            <OutsideClickHandler onOutsideClick={() => modalContext.setShow(false)}>
                <div className="border border-reddit_dark-brightest w-3/4 sm:w-1/2 lg:w-1/4 bg-reddit_dark p-5 text-reddit_text self-center mx-auto rounded-md">
                    ...
                    {modalType === 'login' && (
                        <Button className="w-full py-2 mb-3" style={{ borderRadius: '.3rem' }} onClick={() => login()}>Login In</Button>
                    )}

                    {modalType === "register" && (
                        <Button onClick={e => register(e)} style={{ borderRadius: '.3rem' }} className="w-full py-2 mb-3" > Sign Up</Button>
                    )}
